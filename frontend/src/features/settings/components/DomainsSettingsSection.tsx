...
type DomainsSettingsSectionProps = {
  ...
  customDomains: DomainRecordResponse[];
  isLoading: boolean;
  addPending: boolean;
  ...
  t: (key: string) => string;
  getDomainStateLabel: (state: DomainRecordResponse["state"]) => string;
        ...
        <div className="grid gap-2 md:grid-cols-[1fr_auto]">
          <label className="block text-sm text-[var(--color-label)]">
            ...
            <input
              ...
              disabled={addPending || patchPending}
            />
          </label>
          <div className="flex items-end pb-1">
            <Button type="button" onClick={onAdd} disabled={!customDomainInput.trim() || addPending || patchPending}>
...
function DnsInstruction({ domain, t, onCopy }: { domain: DomainRecordResponse; t: (key: string) => string; onCopy: (value: string) => void }) {
  return (
