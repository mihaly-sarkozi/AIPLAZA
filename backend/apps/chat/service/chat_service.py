...
class ChatService:
    ...
    def __init__(
        ...
        self._llm_answer_service = LLMAnswerService.from_settings(
            client=chat_model,
            chat_model_name=chat_model_name,
            client_factory=self._openai_client,
        )
        self.client = self._llm_answer_service.client
        self.chat_model_name = self._llm_answer_service.chat_model_name
        self._chat_completion_timeout_sec = self._llm_answer_service.completion_timeout_sec
        self._chat_context_timeout_sec = max(
            5,
            int(getattr(settings, "chat_context_timeout_sec", 20) or 20),
        )
        self._chat_max_tokens = self._llm_answer_service.chat_max_tokens
        self._chat_temperature = self._llm_answer_service.chat_temperature
        ...
        self._query_enrichment_service = ChatQueryEnrichmentService()
        self._pii_chat_guard = PiiChatGuardService(
            pii_depersonalization_service=lambda: self.pii_depersonalization_service,
            audit_service=lambda: self.audit_service,
            insufficient_context_answer=self._insufficient_context_answer,
        )
        self._context_packet_service = ChatContextPacketService(
            query_enrichment_service=self._query_enrichment_service,
            max_context_blocks=self._MAX_CONTEXT_BLOCKS,
            max_primary_assertions=self._MAX_PRIMARY_ASSERTIONS,
            max_supporting_assertions=self._MAX_SUPPORTING_ASSERTIONS,
            max_evidence_lines=self._MAX_EVIDENCE_LINES,
            max_context_chunks=self._MAX_CONTEXT_CHUNKS,
            max_context_text_chars=self._MAX_CONTEXT_TEXT_CHARS,
            max_context_block_snippet_chars=self._MAX_CONTEXT_BLOCK_SNIPPET_CHARS,
            ...
            context_timeout_sec=self._chat_context_timeout_sec,
        )
        ...
        self._llm_budget_manager = LlmBudgetManager.from_settings(
        ...
        self._retrieval_context_builder = RetrievalContextBuilder(
            ...
            enrich_parsed_query=self._query_enrichment_service.enrich,
            is_followup=self._query_enrichment_service.is_followup,
            llm_context_text_from_packet=self._context_packet_service.llm_text,
            stamp_packet_kb=self._context_packet_service.stamp_packet_kb,
            merge_context_packets=self._context_packet_service.merge_context_packets,
        )
        self._context_packet_service.set_retrieval_context_builder(self._retrieval_context_builder)
        self._debug_payload_builder = ChatDebugPayloadBuilder(dedupe_keep_order=dedupe_keep_order)
        self._answer_post_processor = AnswerPostProcessor(
            sanitize_debug_text=self._debug_payload_builder.sanitize_text,
            context_text_from_packet=self._context_packet_service.context_text,
            build_messages=self._build_messages,
            build_prompt_context_payload=self._build_prompt_context_payload,
            build_debug_payload=self._debug_payload_builder.build,
            kb_pii_settings=self._pii_chat_guard.kb_pii_settings,
            pii_depersonalization_service=lambda: self.pii_depersonalization_service,
            max_answer_chars=self._chat_max_answer_chars,
            insufficient_context_answer=self._insufficient_context_answer,
        )
        self._chat_stream_service = ChatStreamService(
            context_packet_service=self._context_packet_service,
            pii_chat_guard=self._pii_chat_guard,
            llm_answer_service=self._llm_answer_service,
            build_messages=self._build_messages,
            fold_text=self._fold_text,
            context_timeout_sec=self._chat_context_timeout_sec,
            max_answer_chars=self._chat_max_answer_chars,
        )
        self._chat_with_sources_service = ChatWithSourcesService(
            build_context_packet=self._build_context_packet,
            llm_context_text_from_packet=self._llm_context_text_from_packet,
            pii_chat_guard=self._pii_chat_guard,
            llm_answer_service=self._llm_answer_service,
            answer_post_processor=self._answer_post_processor,
            build_messages=self._build_messages,
            build_prompt_context_payload=self._build_prompt_context_payload,
            knowledge_payload=self._knowledge_payload,
            should_return_direct_knowledge_answer=self._should_return_direct_knowledge_answer,
            looks_broad_enumeration_request=self._looks_broad_enumeration_request,
            policy_violation_error=ChatPolicyViolationError,
            fold_text=self._fold_text,
            kb_service=lambda: self.kb_service,
            context_timeout_sec=self._chat_context_timeout_sec,
            max_answer_chars=self._chat_max_answer_chars,
            enumeration_policy_detail=self._ENUMERATION_POLICY_DETAIL,
        )
