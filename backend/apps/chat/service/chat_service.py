...
class ChatService:
    ...
    def __init__(
        ...
        if not app_settings.openai_api_key and chat_model is None:
            raise ValueError("❌ OPENAI_API_KEY nincs beállítva (config / .env).")
        self.client = chat_model or AsyncOpenAI(api_key=app_settings.openai_api_key)
    ...
    async def _safe_context_text(
        ...
        try:
            ...
            return self._context_text_from_packet(packet), False
