...
def scan_with_clamav(raw: bytes) -> tuple[bool, str]:
    socket_path = str(getattr(settings, "upload_clamav_unix_socket_path", "") or "").strip()
    if not socket_path:
        raise RuntimeError("ClamAV socket path is missing.")
    timeout_sec = max(1, int(getattr(settings, "upload_malware_scan_timeout_sec", 5) or 5))
...
def scan_file_with_clamav(fileobj: BinaryIO) -> tuple[bool, str]:
    socket_path = str(getattr(settings, "upload_clamav_unix_socket_path", "") or "").strip()
    if not socket_path:
        raise RuntimeError("ClamAV socket path is missing.")
    timeout_sec = max(1, int(getattr(settings, "upload_malware_scan_timeout_sec", 5) or 5))
