...
from apps.knowledge.errors import KnowledgePermissionDenied, KnowledgeRetrievalUnavailable, KnowledgeValidationError
...
from apps.knowledge.service.retrieval_build_resolver import RetrievalBuildResolver
...
class RetrievalServiceDependencies:
    def __init__(
        self,
        *,
        ...
        order_chunks_by_vector_hits: Callable[..., list[dict[str, Any]]],
        semantic_blocks_from_vector_hits: Callable[..., list[dict[str, Any]]],
        select_semantic_blocks_for_query: Callable[..., list[dict[str, Any]]],
        filter_relevant_semantic_blocks: Callable[..., list[dict[str, Any]]],
        semantic_blocks_context: Callable[..., str],
        ...
        self._knowledge_feedback_service = knowledge_feedback_service
        ...
        self._order_chunks_by_vector_hits = order_chunks_by_vector_hits
        self._semantic_blocks_from_vector_hits = semantic_blocks_from_vector_hits
        self._select_semantic_blocks_for_query = select_semantic_blocks_for_query
        self._filter_relevant_semantic_blocks = filter_relevant_semantic_blocks
        self._semantic_blocks_context = semantic_blocks_context
...
class RetrievalService:
    def __init__(
        ...
        *,
        ...
        dependencies: "RetrievalServiceDependencies | None" = None,
    ) -> None:
        ...
        if dependencies is None:
            dependencies = RetrievalServiceDependencies(
                ...
                metrics_store=None,
                index_build_store=None,
                query_run_store=None,
                context_builder=None,
                knowledge_feedback_service=None,
                lineage_service=None,
                load_existing_global_profiles=lambda **_kwargs: [],
                load_existing_retrieval_chunks=lambda **_kwargs: [],
                load_existing_semantic_blocks=lambda **_kwargs: [],
                order_chunks_by_vector_hits=lambda chunks, _hits: chunks,
                semantic_blocks_from_vector_hits=lambda **_kwargs: [],
                select_semantic_blocks_for_query=lambda **_kwargs: [],
                filter_relevant_semantic_blocks=lambda **_kwargs: [],
                semantic_blocks_context=lambda **_kwargs: "",
                log_step=lambda *_args, **_kwargs: None,
            )
        for name, value in dependencies.__dict__.items():
            setattr(self, name, value)
        self._resilience_runner: RetrievalResilienceRunner | None = None
        self._build_resolver: RetrievalBuildResolver | None = None

    def _runner(self) -> RetrievalResilienceRunner:
        if self._resilience_runner is None:
            self._resilience_runner = RetrievalResilienceRunner(
                retrieval_engine=self._retrieval_engine,
                metrics_store=self._metrics_store,
                timeout_seconds=_RETRIEVAL_TIMEOUT_SECONDS,
                retry_attempts=_RETRIEVAL_RETRY_ATTEMPTS,
                retry_backoff_seconds=_RETRIEVAL_RETRY_BACKOFF_SECONDS,
            )
        return self._resilience_runner

    def _builds(self) -> RetrievalBuildResolver:
        if self._build_resolver is None:
            self._build_resolver = RetrievalBuildResolver(index_build_store=self._index_build_store)
    ...
    def _require_tenant_scoped_corpus(self, *, tenant: str, corpus_uuid: str):
        ...
        if not effective_tenant:
            raise KnowledgePermissionDenied("Tenant scope is required for knowledge retrieval.")
    ...
    async def retrieve(
        ...
        *,
        ...
        if self._retrieve_query is None:
            raise KnowledgeRetrievalUnavailable("Retrieval query runner is not configured.")
    ...
    async def retrieve_hits_with_resilience(
        ...
        *,
        ...
        return await self._runner().retrieve_hits(
            tenant=tenant,
            corpus_uuid=corpus_uuid,
            query=query,
            builds=builds,
    ...
    async def retrieve_query_run(
        ...
        *,
        ...
        builds = self._builds().resolve_builds(corpus_uuid=corpus_uuid, build_ids=build_ids)
    ...
    async def build_chat_context(
        ...
        *,
        ...
        if not effective_query:
            raise KnowledgeValidationError("Query is required for chat context build.")
        if not effective_corpus_uuid:
            raise KnowledgeValidationError("Corpus UUID is required for chat context build.")
