# Ez a fájl a(z) multilingual_analyzer modul backend logikáját tartalmazza.
"""
...
def _strictly_contains_span(outer: DetectionResult, inner: DetectionResult) -> bool:
    """True, ha az outer span szigorúan tartalmazza az inner-t (nem azonos hossz)."""
    if outer.start > inner.start or outer.end < inner.end:
        return False
    return (outer.end - outer.start) > (inner.end - inner.start)
...
def merge_and_dedupe(detections: List[DetectionResult]) -> List[DetectionResult]:
    ...
    for d in sorted_d:
        ...
        if prefer_d and not keep_best:
            # IBAN / bankszámla teljes span: ne írja felül egy belső, részleges jármű-találat (pl. context_number)
            if d.entity_type == EntityType.VEHICLE_REGISTRATION and any(
                m.entity_type in (EntityType.IBAN, EntityType.BANK_ACCOUNT_NUMBER)
                and _strictly_contains_span(m, d)
