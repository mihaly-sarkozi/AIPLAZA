# Ez a fájl a(z) context_number_recognizer modul backend logikáját tartalmazza.
"""
...
# Ez a függvény visszaadja a(z) kontexus logikáját.
...
# Ez a függvény a(z) is_date_shaped logikáját valósítja meg.
def _is_date_shaped(matched: str) -> bool:
    clean = re.sub(r"\s+", "", matched)
    return bool(_DATE_SHAPE_PATTERN.match(clean))


# Ez a függvény a(z) has_year_month_day_triplet logikáját valósítja meg.
