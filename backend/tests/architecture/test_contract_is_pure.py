...
CONTRACT_ROOT = BACKEND_ROOT / "core" / "platform" / "contract"
FORBIDDEN_PREFIXES = (
    "fastapi",
    "sqlalchemy",
...
def _is_forbidden_contract_import(module: str) -> str | None:
...
def test_contract_package_uses_only_pure_imports() -> None:
    violations: list[str] = []

    for path in iter_python_files(CONTRACT_ROOT):
        for occurrence in collect_imports(path):
            reason = _is_forbidden_contract_import(occurrence.imported_module)
            if reason is None:
                continue
            violations.append(describe_violation(occurrence, detail=reason))

    assert not violations, format_violations(
        rule="Architektúra-szabály sérült: a `core/platform/contract` csak tiszta contract típusokat és hookokat tartalmazhat.",
        guidance="A contract réteg ne importáljon runtime-ot, repository-t, modelt, ORM-et vagy frameworköt; csak típusokat, interfészeket, route contractokat és service key-ket használj.",
