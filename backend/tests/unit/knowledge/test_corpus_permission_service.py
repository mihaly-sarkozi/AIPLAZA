...
@pytest.mark.parametrize(
    ("method_name", "resource", "allowed_user", "denied_user"),
    [
        ("can_view_knowledge_base", lambda store: store.kbs[0], SimpleNamespace(id=11, tenant="demo", role="user"), SimpleNamespace(id=99, tenant="demo", role="user")),
        ("can_train_knowledge_base", lambda store: store.kbs[0], SimpleNamespace(id=11, tenant="demo", role="user"), SimpleNamespace(id=12, tenant="demo", role="user")),
        ("can_delete_knowledge_base", lambda store: store.kbs[0], SimpleNamespace(id=11, tenant="demo", role="user"), SimpleNamespace(id=12, tenant="demo", role="user")),
        ("can_view_ingest_run", lambda store: SimpleNamespace(corpus_uuid="kb-1", tenant="demo"), SimpleNamespace(id=11, tenant="demo", role="user"), SimpleNamespace(id=99, tenant="demo", role="user")),
        ("can_reprocess_ingest_item", lambda store: SimpleNamespace(corpus_uuid="kb-1", tenant="demo"), SimpleNamespace(id=11, tenant="demo", role="user"), SimpleNamespace(id=12, tenant="demo", role="user")),
        ("can_delete_source", lambda store: SimpleNamespace(corpus_uuid="kb-1", tenant="demo"), SimpleNamespace(id=11, tenant="demo", role="user"), SimpleNamespace(id=12, tenant="demo", role="user")),
        ("can_start_index_build", lambda store: store.kbs[0], SimpleNamespace(id=11, tenant="demo", role="user"), SimpleNamespace(id=12, tenant="demo", role="user")),
    ],
)
def test_knowledge_permission_methods_have_allowed_and_denied_cases(method_name, resource, allowed_user, denied_user) -> None:
    store = _CorpusStore()
    service = _knowledge_permission_service(store)
    target = resource(store)

    assert getattr(service, method_name)(allowed_user, target) is True
    assert getattr(service, method_name)(denied_user, target) is False


@pytest.mark.parametrize(
    ("method_name", "resource"),
    [
        ("can_view_knowledge_base", lambda store: store.kbs[0]),
        ("can_train_knowledge_base", lambda store: store.kbs[0]),
        ("can_delete_knowledge_base", lambda store: store.kbs[0]),
        ("can_view_ingest_run", lambda store: SimpleNamespace(corpus_uuid="kb-1", tenant="demo")),
        ("can_reprocess_ingest_item", lambda store: SimpleNamespace(corpus_uuid="kb-1", tenant="demo")),
        ("can_delete_source", lambda store: SimpleNamespace(corpus_uuid="kb-1", tenant="demo")),
        ("can_start_index_build", lambda store: store.kbs[0]),
    ],
)
def test_knowledge_permission_methods_deny_cross_tenant_access(method_name, resource) -> None:
    store = _CorpusStore()
    service = _knowledge_permission_service(store)
    cross_tenant_user = SimpleNamespace(id=11, tenant="other", role="user")

    assert getattr(service, method_name)(cross_tenant_user, resource(store)) is False
...
def test_knowledge_permission_methods_allow_owner_scope(method_name, resource) -> None:
    store = _CorpusStore()
    service = _knowledge_permission_service(store)
    owner = SimpleNamespace(id=99, tenant="other", role="owner")

    assert getattr(service, method_name)(owner, resource(store)) is True


def test_knowledge_metrics_permission_has_allowed_denied_and_scope_cases(monkeypatch: pytest.MonkeyPatch) -> None:
    service = _knowledge_permission_service(_CorpusStore())
    scoped_user = SimpleNamespace(id=4, role="user")
    monkeypatch.setattr(
        "apps.knowledge.service.knowledge_permission_service.has_permission",
        lambda user, permission: bool(user is scoped_user and permission == "knowledge.metrics.view"),
    )

    assert service.can_view_knowledge_metrics(SimpleNamespace(id=1, role="owner")) is True
    assert service.can_view_knowledge_metrics(SimpleNamespace(id=2, role="admin")) is True
    assert service.can_view_knowledge_metrics(scoped_user) is True
    assert service.can_view_knowledge_metrics(SimpleNamespace(id=3, role="user")) is False
