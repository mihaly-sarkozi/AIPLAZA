"""Demo signup orchestráció (use case-ek + vékony orchestrator + facade).

Kapcsolódó, külön csomagokban:
  - ``tenant.provisioning`` – modell, validátor, ``TenantProvisioningService``
  - ``tenant.slug``         – slug policy + ``DemoSlugReserver``
  - ``tenant.tokens``       – ``DemoLoginTokenService``
  - ``tenant.signup.errors`` – típusos signup kivételek
"""
