...
--------------------------
  ...
  5. tenant       – PLATFORM_USERS_SERVICE, PLATFORM_CLOCK_SERVICE
                    (opcionálisan: platform usage service – lazy callable, nem blokkoló)
  6. domain       – PLATFORM_TENANT_LIFECYCLE_POLICY (tenant modul regisztrálja)
  7. brand        – PLATFORM_TENANT_LIFECYCLE_POLICY (tenant modul regisztrálja)
...
def load_core_platform_manifest() -> PlatformManifest:
    ...
    platform_modules = (
        ...
        # 5. Tenant: PLATFORM_USERS_SERVICE + PLATFORM_CLOCK_SERVICE szükséges
        #    Opcionális: platform usage service – lazy callable, app modul regisztrálja
        get_tenant_platform_module(),
        # 6. Domain: PLATFORM_TENANT_LIFECYCLE_POLICY szükséges (tenant regisztrálja)
        get_domain_platform_module(),
        # 7. Brand: PLATFORM_TENANT_LIFECYCLE_POLICY szükséges (tenant regisztrálja)
