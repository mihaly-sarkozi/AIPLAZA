...
# `settings` is imported lazily inside get_password_policy() so that the pure
# PasswordPolicy data-class, PASSWORD_POLICIES dict, and validate_* functions
# are all importable without loading the configuration subsystem – enabling
# unit tests that don't need a running environment.
