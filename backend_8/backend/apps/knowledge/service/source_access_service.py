...
from apps.knowledge.errors import IngestInputNotFound, KnowledgeSourceUnavailable
...
class SourceAccessService:
    ...
    def read_ingest_file_bytes(self, item_id: str) -> tuple[bytes, str | None, str | None]:
        ...
        if ingest_input is None:
            raise IngestInputNotFound()
        if ingest_input.input_type != "file":
            raise KnowledgeSourceUnavailable("Ingest input is not a file.")
        if not ingest_input.bucket_name or not ingest_input.object_key:
            raise KnowledgeSourceUnavailable("Object storage reference is missing.")
