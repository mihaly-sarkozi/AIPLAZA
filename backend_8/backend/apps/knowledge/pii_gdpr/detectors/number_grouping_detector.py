# Ez a fájl a(z) number_grouping_detector modul backend logikáját tartalmazza.
"""
...
# Ugyanaz a mag mint a regex_detector IBAN sora; teljes blokkra (ne a távoli „rendszám” szó döntsön).
_IBAN_SPAN_FULLMATCH = re.compile(
    r"^[A-Z]{2}\d{2}\s?(?:[A-Z0-9]\s?){4}(?:[A-Z0-9]\s?){4,28}$",
    re.I,
)


def _looks_like_iban_span(matched: str) -> bool:
    """IBAN-formátumú összevont számjegy-blokk (pl. egy mondatban később is szerepelhet „rendszám”)."""
    s = matched.strip().rstrip(".,;:()[]{}<>\"'")
    return bool(_IBAN_SPAN_FULLMATCH.match(s))
...
class Token:
    ...
    # Ez a metódus a(z) has_digit logikáját valósítja meg.
    @property
    def has_digit(self) -> bool:
        return bool(_HAS_DIGIT.search(self.text))

    # Ez a metódus a(z) is_connector logikáját valósítja meg.
    @property
    def is_connector(self) -> bool:
        return bool(_CONNECTOR_WORDS.match(self.text))


# Ez a függvény a(z) tokenize logikáját valósítja meg.
...
# Ez a függvény a(z) is_likely_year logikáját valósítja meg.
...
class NumberGroupingDetector(BaseDetector):
    ...
    def detect(self, text: str, language: str = "en") -> List[DetectionResult]:
        ...
        for group in groups:
            ...
            if _looks_like_iban_span(matched):
                results.append(
                    DetectionResult(
